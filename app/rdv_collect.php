          ...
          <tr> 
            ...
            <td> <select name="estacion">
			  ...
			  ?>
			    >Todas</option>
                <?php
		  	$sql="select est_id,est_codigo,est_nombre "
				."from estacion "
				."order by est_codigo";
			$rs = &$conn->Execute($sql);
			while(!$rs->EOF)
		  ...
		  <TD>
		    <TABLE WIDTH='100%' border=0 CELLPADDING=2 CELLSPACING=1 BGCOLOR='#CCCCCC'>
                <TR BGCOLOR="#CCCCCC"> 
                  ...
                  <td width="17%" nowrap class='table_hd'>Nro Facturas</td>                  
                  <td width="32%" nowrap class='table_hd'>Ver Reporte (Correspondencia 
                    y Carga)</td>
                </tr>
                <?php
		$tipo_guia="CO"; //collect		
				
		while(!$rs->EOF)
		{
		    $vest_id=$rs->fields[0];
		    $vest_codigo=$rs->fields[1];
		    $vest_nombre=$rs->fields[2];
			$vtexto=$vest_codigo." : ".$vest_nombre;
			
			//calcular nro de facturas
			$sql1="select count(gui_id)  "
				."from guia "
				."where est_origen=$vest_id "
...
?>
                <TR valign=top bgcolor='#ffffff'> 
                  ...
                  <TD valign=top nowrap> <a href="rdv_collect1.php?est_id=<?=$vest_id?>&fecha_ini=<?=$fecha_ini?>&fecha_fin=<?=$fecha_fin?>" target="_blank">Click 
                    para Ver</a> </TD>
                </TR>
                <?php
			$rs->MoveNext();
