...
class c_impdescarxdoc
{
  ...
  function c_impdescarxdoc($conDb,$usuario)
  {
	  $this->sto_id="";
	  $this->ofi_id="";
	  $this->stotip_id="";
  ...
  function cargar_dato($dato,$iou="i")
  {
	  ...
	  if($ncampos==count($dato))
	  {
	    ...
	    $this->sto_fechaexp=$dato[4];
	    $this->stoofi_ini=$dato[5];
	    $this->stoofi_fin=$dato[6];
	    $this->stoofi_actual=$dato[7];
	    /*
	  ...
	  if($ncampos==count($dato))
	  {
	    ...
	    $this->sto_fechaexp=$dato[1];
	    $this->stoofi_ini=$dato[2];
	    $this->stoofi_fin=$dato[3];
	    $this->stoofi_actual=$dato[4];
	    $res=1;
	  }
	  else
	    $res=0;
...
va;
    ...
    $fechaLarga=$this->oParametro->fechaLarga;
	  ...
	  to_char(s.sto_fecha,'$fechaCorta') fecha,to_char(s.sto_fechaexp,'$fechaCorta') fecha exp,
	  s.stoofi_ini,s.stoofi_fin,s.stoofi_actual,
	  $cadId
	  from impdescarxdoc s, oficina o, stock_tipo t
...
va;
  ...
  {
    $oAux=new c_impdescarxdoc($this->con);
  	$oAux->info($id);
	$campo=array(
				...
				array("etiqueta"=>"* Actual","nombre"=>"tActual","tipo_campo"=>"tet","sql"=>"","valor"=>$oAux->stoofi_actual)
...
va;
  ...
  {
	echo "<hr>Class c_impdescarxdoc(conDb)<br>";
...
va;
  ...
  function add()
  {
  	$oAux=new c_impdescarxdoc($this->con,$this->usu_audit);
  	$this->sto_id=$oAux->nuevoStoId($this->ofi_id,$this->sto_id);
...
va;
  ...
  {
    $cadId=$this->armarSqlId("","");
    $cad="select $cadId a1,$cadId a2 from impdescarxdoc order by a1";
    return($cad);
  }
