...
/**
 * Clase usada para el manejo de pais
 *
 */
class c_pais
{
   ...
   * @return c_pais
   */
  function c_pais($conDb)
  {
	  $this->pai_codigo="";
	  $this->pai_descripcion="";
...
va;
  	...
  	$sql=<<<va
  	  select pai_codigo a1,
	  pai_codigo a2,pai_descripcion,
	  pai_codigo a3
	  from pais 
	  order by a1,pai_descripcion
va;
...
va;
  ...
  {
	echo "<hr>Class c_pais(conDb)<br>";
  	echo "pai_codigo:".$this->pai_codigo."<br>";
	echo "pai_codigo:".$this->pai_codigo."<br>";
	echo "<hr>";
  }
  ...
  function existe($vid)
  {
  	$sql="select pai_codigo from pais where pai_codigo='$vid'";
	$rs = &$this->con->Execute($sql);
	if($rs->EOF)
	  $res="0";
	else 
  ...
  { 
    $sql="select nvl(max(to_number(pai_codigo)),0) from pais";
    $rs=&$this->con->Execute($sql);
    $res=$rs->fields[0];
    $nuevo=$res+1;
    return($nuevo);
  ...
  */
  ...
  function add()
  {
  	$existe=$this->existe($this->pai_codigo);
	if($existe=="0")
	{
	  $sql=<<<va
	  insert into pais
	  (
	    pai_codigo,pai_descripcion
	  )
	  values 
	  (
	    '$this->pai_codigo','$this->pai_descripcion'
	  )
va;
	  $rs = &$this->con->Execute($sql);
	  if($rs)
...
va;
  ...
  function update($id)
  {
	...
	if($existe!="0")
	{
	  ...
	  UPDATE pais 
	  set 
	  pai_descripcion='$this->pai_descripcion',
...
va;
	...
	{
	  $res=$id;
	  $this->pai_codigo=$rs->fields[0];
	  $this->pai_descripcion=$rs->fields[1];
	  $this->borrar=$rs->fields[2];
	  $this->borrar=$rs->fields[3];
	  $this->borrar=$rs->fields[4];
	  $this->borrar=$rs->fields[5];
	}
	return($res);
  }
